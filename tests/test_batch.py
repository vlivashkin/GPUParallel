...
class TestBatchGPUParallel(unittest.TestCase):
    ...
    def test_batched_results_equal(self):
        ...
        non_batched_results = list(non_batched_results)
        del gpup
    ...
    def test_args_flat_result(self):
        ...
        bgpup = BatchGPUParallel(
            task_fn=task_return_identity, batch_size=3, flat_result=True, n_gpu=2, progressbar=False
        )
    ...
    def test_nonbatched_args(self):
        ...
        bgpup = BatchGPUParallel(
            task_fn=task_return_all_kwargs, batch_size=3, flat_result=False, n_gpu=1, progressbar=False
        )
    ...
    def test_run_twice(self):
        for _ in range(2):
            arr = np.zeros((102, 103))
            bgpup = BatchGPUParallel(
                task_fn=task_return_identity, batch_size=2, flat_result=True, n_gpu=16, progressbar=False
            )
            flat_results = np.array(list(bgpup(arr)))
            self.assertEqual(arr.shape, flat_results.shape)
            del bgpup
